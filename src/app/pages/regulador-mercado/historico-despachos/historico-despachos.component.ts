import { Observable, reduce, switchMap, from, map, forkJoin } from 'rxjs';
...
export class HistoricoDespachosComponent implements OnInit, OnDestroy {
  ...
  cantidadEnergiaDepachada: number;
  ...
  private getHistoricoDespachos() {
    ...
    observers.push(this.despachosEnergia.getHistoricoDespachosEnergia());

    //Total de energia despachada
    observers.push(this.despachosEnergia.getHistoricoDespachosEnergia().pipe(
      switchMap((data) => {
        return from(data).pipe(
          map(data => data.cantidadEnergia),
          reduce((acc, curr) => acc + curr)
        )
      })
    ));

    //Total de energia inyectada
    observers.push(this.despachosEnergia.getHistoricoDespachosEnergia().pipe(
      switchMap((data) => {
        return from(data).pipe(
          map(data => data.cantidadProducida),
          reduce((acc, curr) => acc + curr)
        )
      })
    ));
